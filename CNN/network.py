...
import CNN.layer as layer
import CNN.filter_ops as filter
import CNN.accelerated_layer as layer_accel
...
class Network:
    ...
    def fit(self, x_train, y_train, x_validate, y_validate, mini_batch_sz=100, n_epochs=10, acc_freq=10, print_every=50):
        ...
        n_iter = n_minibatches * n_epochs
        
        loss_history = []
        acc_val = []
        acc_train = []
        #if self.verbose: 
        print(f"Starting to train network...There will be {n_epochs} epochs and {n_iter} iterations total, {n_minibatches} iter/epoch.")

        start = time.time()
        for current_epoch in range(n_epochs):
            for i in range(n_minibatches):
                idx = np.random.choice(n, mini_batch_sz, replace = True)
                x_mini = x_train[idx]
                y_mini = y_train[idx]
                loss = self.forward(x_mini, y_mini)
                ...
                #if self.verbose:
                current_iter = current_epoch * (n_minibatches) + i
                if current_iter == 0:
                    time_first = (time.time() - start) / 60
                    print(f"Runtime of first iteration:{time_first} minutes. Total time expected: {time_first * n_iter} minutes.")
                if current_iter % print_every == 0: 
                    print(f"Completed iter {current_iter}/{n_iter}. Training loss: {loss_history[-1]:.2f}.")
                if current_iter % acc_freq == 0:
                    print(f"\tValidation Accuracy: {self.accuracy(x_validate, y_validate, mini_batch_sz = mini_batch_sz) * 100:.1f}%")

            #Completed Epoch, recording accuracy
            # may be useful later
            #acc_val.append(self.accuracy(y_validation, self.predict(x_validation)))
            #acc_train.append(self.accuracy(y_mini, self.predict(features_mini)))
    ...
    def wt_reg_reduce(self):
        ...
        wt_reg = 0
        for layer in self.layers:
            if layer.wts is not None:
                wt_reg += np.sum(layer.wts)
        
        return wt_reg * self.reg
...
class ConvNet4(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
                 pooling_sizes=(2,), pooling_strides=(2,), n_classes=10, wt_scale=1e-3, reg=0, verbose=True):
        n_kers: tuple. Number of kernels/units in  1st convolution layer. Format is (32,), which is a tuple
        ...
        self.layers.append(layer.Conv2D(0, "ConvLayer", n_kers[0], ker_sz[0], n_chans, wt_scale, 'relu', reg, verbose))
        # 2) 2x2 max pooling layer
        self.layers.append(layer.MaxPooling2D(1, "MaxPool", pooling_sizes[0], pooling_strides[0], reg=reg, verbose=verbose))
        # 3) Dense layer
        #calculating the output size of pooling layer
        pool_out_size = filter_ops.get_pooling_out_shape(input_shape[1], pooling_sizes[0], pooling_strides[0])
        pool_out_size *= filter_ops.get_pooling_out_shape(input_shape[2], pooling_sizes[0], pooling_strides[0])
        pool_out_size *= n_kers[0]
        self.layers.append(layer.Dense(2, "Dense1_relu", dense_interior_units[0], pool_out_size, wt_scale, "relu", reg, verbose))
        # 4) Dense softmax output layer
        self.layers.append(layer.Dense(3, 'DenseOutput_softmax', n_classes, dense_interior_units[0], wt_scale, 'softmax', reg, verbose))
...
class ConvNet4Accel(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
        ...
        # Input
        self.layers.append(layer_accel.Conv2DAccel(0, "ConvLayer", n_kers[0], ker_sz[0], n_chans, wt_scale, 'relu', reg, verbose))
        # 2) 2x2 max pooling layer
        self.layers.append(layer_accel.MaxPooling2DAccel(1, "MaxPool", pooling_sizes[0], pooling_strides[0], reg=reg, verbose=verbose))
        # 3) Dense layer
        #calculating the output size of pooling layer
        pool_out_size = filter_ops.get_pooling_out_shape(input_shape[1], pooling_sizes[0], pooling_strides[0])
        pool_out_size *= filter_ops.get_pooling_out_shape(input_shape[2], pooling_sizes[0], pooling_strides[0])
