...
class Adam(Optimizer):
    ...
    def update_weights(self):
        ...
        self.p = self.beta2 * self.p + (1 - self.beta2) * np.square(self.d_wts)
        vc = self.v / (1 - np.power(self.beta1, self.t ))
        pc = self.p / (1 - np.power(self.beta2, self.t ))
        self.t += 1
